...
{
    "location": "types/#NonlinearEigenproblems.NEPTypes.SPMF_NEP-Tuple{Array{#s271,1} where #s271<:(AbstractArray{T,2} where T),Array{#s270,1} where #s270<:Function}",
    "page": "NEP Types",
    "title": "NonlinearEigenproblems.NEPTypes.SPMF_NEP",
    "category": "method",
...
{
    "location": "types/#NonlinearEigenproblems.NEPTypes.REP-Tuple{Any,Array{#s271,1} where #s271<:Number}",
    "page": "NEP Types",
    "title": "NonlinearEigenproblems.NEPTypes.REP",
    "category": "method",
...

{
    ...
    "category": "section",
    "text": "The technique is based on moving the boundary condition at L_1 to L_0. This can be done without doing any approximation, if we allow the new artificial boundary condition at L_1 to depend on λ. The technique is called absorbing boundary conditions.We first note that we can transform the problem to first order form  fracddx\nbeginbmatrixpsi(x)psi(x)endbmatrix\n=\nbeginbmatrix\n0  1\nlambda+V(x)  0\nendbmatrix\nbeginbmatrixpsi(x)psi(x)endbmatrixThe potential V(x) is constant in the domain L_0L_1. This  allows us to directly express the solution using the matrix exponentialbeginbmatrixpsi(x)psi(x)endbmatrix\n=expleft((x-L_0)\nbeginbmatrix\n0  1\nlambda+V_0  0\nendbmatrix\nright)\nbeginbmatrixpsi(L_0)psi(L_0)endbmatrixwhen xinL_0L_1. The boundary condition psi(L_1)=0 can be imposed as0=psi(L_1)=beginbmatrix1  0endbmatrix\nbeginbmatrixpsi(L_1)psi(L_1)endbmatrix\n=beginbmatrix1  0endbmatrixexpleft((L_1-L_0)\nbeginbmatrix\n0  1\nlambda+V_0  0\nendbmatrix\nright)\nbeginbmatrixpsi(L_0)psi(L_0)endbmatrixBy explicitly computing the matrix exponential of the antidiagonal two-by-two matrix we obtain the relation0=\ng(λ)psi(L_0)+\nf(λ)psi(L_0)whereg(λ)=coshleft((L_1-L_0)sqrtλ+V_0right)f(λ)=fracsinhleft((L_1-L_0)sqrtλ+V_0right)sqrtλ+V_0A solution to the original boundary value problem must satisfy the condition 0=g(λ)psi(L_0)+f(λ)psi(L_0) in the middle of the domain. If we now consider only the domain 0L_0, we see that the function must satisfybegineqnarray*\n left(\n fracpartial^2partial x^2\n-V(x)-lambda\nright)psi(x)=0 xin0L_0\n   psi(0)=0\n   g(λ)psi(L_0)+f(λ)psi(L_1)=0\nendeqnarray*which is a boundary value problem with a mixed boundary condition (since it contains both psi(L_0) and psi(L_0)). In general, the solutions to the original problem are the same as the solutions on the reduced domain."
},

{
    ...
    "category": "section",
    "text": "The λ-dependent boundary value problem can now be discretized in the domain 0L_0 with usual techniques, e.g., with finite difference as follows.Let x_k=hk, k=1ldots n and h=1n such that x_1=h and x_n=1=L_0. Approximation of the lambda-dependent boundary condition can be achieved with the a one-sided second order difference scheme   0=g(λ)psi(L_0)+f(λ)frac1hleft(frac32 psi(L_0)\n-2psi(x_n-1)\n+frac12psi(x_n-2)right)+O(h^2)Let  D_n=\nfrac1h^2\nbeginbmatrix\n-2   1  0 \n1  ddots 1 \n0  1 -2  1\n0  cdots  0  0\nendbmatrixtextrm and \nunderlineI_n=beginbmatrix1  ddots 1    0endbmatrixThen the boundary value problem can expressed as M(λ)v=0 whereM(λ)=D_n-operatornamediag(V(x_1)ldotsV(x_n-1)0)-λunderlineI_n\n+g(λ)e_ne_n^T+f(λ)FandF=frac12he_ne_n-2^T-frac2he_ne_n-1^T+frac32he_ne_n^T"
},
